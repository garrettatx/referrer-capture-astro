...
export function clip(value, max = MAX_FIELD) {
    if (typeof value !== 'string')
        return '';
...
export function classifyReferrer(referrer) {
    ...
    if (ai)
        return { source: ai, medium: 'ai-referral' };
...
export function extractClickIds(query) {
    const params = parseQuery(query);
    const found = {};
    for (const id in CLICK_IDS) {
        const value = params.get(id);
        if (value)
            found[id] = clip(value, 300);
    }
    return found;
}
/** True when the referrer is the site itself. */
export function isInternal(referrer, host) {
...
export function classify(input) {
    ...
    if (utmSource || utmMedium) {
        ...
        if (!source && paidClick)
            source = CLICK_IDS[paidClick].source;
        if (!medium && paidClick)
    ...
    if (inferFrom) {
        ...
        return { ...base, src: mapped.source, med: mapped.medium, m: 'click-id' };
    }
    ...
    if (referrer) {
        ...
        return { ...base, src: source, med: medium, m: 'referrer' };
    }
    // 5. No referrer, no tags. Direct.
    return { ...base, src: 'direct', med: 'none', m: 'direct' };
}
/** A touch that carries no attribution signal. */
export function isDirectTouch(touch) {
