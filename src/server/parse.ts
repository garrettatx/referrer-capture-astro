import type { AttributionRecord, NormalizedAttribution } from '../core/types.js';
 ...
 */
export function parseAttribution(input: unknown): NormalizedAttribution {
  try {
    const record = toRecord(input);
    if (!record) return { ...UNATTRIBUTED };
...
function toRecord(input: unknown): AttributionRecord | null {
  if (input == null) return null;

  let value: unknown = input;

  if (typeof input === 'string') {
    if (input.length > MAX_PAYLOAD_BYTES) return null;
    try {
